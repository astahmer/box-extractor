...
const getObjectLiteralPropValue = (
    ...
    const property =
        initializer.getProperty(propName) ??
        initializer.getProperties().find((p) => findProperty(p, propName, stack, ctx));
...
const getElementAccessedExpressionValue = (
    ...
    if (Node.isBinaryExpression(arg)) {
        ...
        const propName =
            tryComputingPlusTokenBinaryExpressionToString(arg, stack, ctx) ?? maybePropName(arg, stack, ctx);
